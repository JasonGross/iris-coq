...
* Strengthen `big_opL_zip_seq` by weakening the premise from `=` to `≤`.
  (by Rudy Peterson)
...
* Strengthen `big_sepL_zip_seq`, `big_andL_zip_seq` and `big_orL_zip_seq` by
  weakening the premise from `=` to `≤`. (by Rudy Peterson)
